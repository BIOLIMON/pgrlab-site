...
export const SectionHeader: React.FC<{
  ...
  <div className={`mb-16 ${align === 'center' ? 'text-center' : 'text-left'}`}>
    ...
    {subtitle && (
      <p className='text-xl text-slate-500 max-w-2xl mx-auto leading-relaxed'>{subtitle}</p>
    )}
  </div>
);
...
export const Card: React.FC<{
  ...
  <div
    className={`bg-white border border-slate-100 rounded-3xl transition-all hover:shadow-xl hover:shadow-slate-200/50 group ${padding ? 'p-10' : ''} ${className}`}
  >
    {children}
  </div>
);
