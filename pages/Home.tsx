    ...
    <>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
          {researchThemes.map((theme) => (
            <Card key={theme.id}>
              ...
              <Link
                to='/research'
                className='text-sm font-bold flex items-center group-hover:text-cyan-600 transition-colors'
              >
                Learn more{' '}
      ...
      <Section className='bg-slate-50'>
        <SectionHeader
          ...
          subtitle='Key contributions to the understanding of plant transcriptional regulation and systems biology.'
        />
        <div className='space-y-6'>
          {featuredPubs.map((pub) => (
            <div
              key={pub.id}
              className='bg-white p-8 md:p-12 rounded-[40px] border border-slate-100 flex flex-col md:flex-row gap-10 items-start md:items-center'
            >
              <div className='flex-grow'>
                <div className='flex flex-wrap gap-2 mb-6'>
                  {pub.tags.map((tag) => (
                    <span
                      key={tag}
                      className='px-3 py-1 bg-slate-100 text-slate-500 rounded-full text-[10px] font-bold uppercase tracking-wider'
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <p className='text-cyan-600 font-bold italic'>
          ...
          <div className='pt-10 text-center'>
            <Link to='/publications'>
              <span className='text-sm font-bold text-slate-400 hover:text-slate-900 cursor-pointer'>
        ...
        <div className='bg-slate-900 rounded-[60px] p-12 md:p-24 text-white relative overflow-hidden'>
          <div className='relative z-10 max-w-4xl mx-auto'>
            ...
            <p className='text-lg text-slate-300 mb-16 text-center max-w-3xl mx-auto leading-relaxed'>
              We collaborate with leading groups in systems biology, plant genomics, and
              computational modeling to understand how plants integrate environmental signals across
              scales.
            </p>
            ...
            <div className='text-center'>
              <Link to='/contact'>
                <Button
                  variant='primary'
                  className='!bg-white/10 !text-white !border !border-white/20 !px-10 !py-5 text-base hover:!bg-white/20 transition-all'
                >
                  Collaborate with us
                </Button>
              </Link>
