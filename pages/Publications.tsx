...
export const Publications: React.FC = () => {
  ...
  const filtered =
    filter === 'All' ? publications : publications.filter((p) => p.tags.includes(filter));
      ...
      <Section>
        ...
        <div className='grid grid-cols-1 gap-8'>
          {filtered.map((pub) => (
            <div
              key={pub.id}
              className='group bg-white hover:bg-slate-50 border border-slate-100 p-8 md:p-10 rounded-[40px] transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-8'
            >
              <div className='max-w-3xl'>
                ...
                <h3 className='text-2xl font-bold mb-4 group-hover:text-cyan-700 transition-colors'>
                  {pub.title}
                </h3>
